...
Concurrent submissions for the same locator and mode join its queued or running generation run. This prevents duplicate regeneration requests from competing to publish the same next result version while preserving regeneration priority over normal work.
