...
func TestLast5GamesReturnsOneEntryPerPlayerFromABatchedPopulation(t *testing.T) {
	...
	run := harness.startAndProcess(t, StartRequest{Target: CardTarget{Template: Last5Games}, MatchID: "m-1", CardState: PreToss, Inputs: map[string]any{"players": []string{"1", "2"}, "format": "t20"}})
...
func TestLast5GamesMarksShortAndMissingPlayerHistory(t *testing.T) {
	...
	run := harness.startAndProcess(t, StartRequest{Target: CardTarget{Template: Last5Games}, MatchID: "m-1", CardState: PreToss, Inputs: map[string]any{"players": []string{"1", "2"}, "format": "t20", "latest_matches": 5}})
	result, err := harness.module.GetCurrentResult(harness.ctx, Last5Games, "m-1", PreToss, run.NormalizedInputs)
